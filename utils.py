...
def get_save_image_path():
    if settings.DEBUG == False:
        ...
        return '%s/tb/static/images' % settings.BASE_DIR
...
def push_redis(guid, user_id, info, type = 'word', save_to_mysql = True):
    data = {
        'id': None,
        'user_id': user_id,
        'info': info,
    ...
    if save_to_mysql:
        ...
        logging.warn('push_redis 1111')

    red.rpush(guid, json.dumps(data))


def create_analysis_table(user_id):
    # 创建分析商品评论结果表
    command = (
        "CREATE TABLE IF NOT EXISTS {} ("
        "`id` INT(5) NOT NULL AUTO_INCREMENT,"  # 自增 id
        "`user_id` BIGINT(15) DEFAULT NULL ,"  # 商品 id
        ...
        "PRIMARY KEY(id)"
        ") ENGINE=InnoDB".format(config.analysis_item_table + '_' + user_id))
