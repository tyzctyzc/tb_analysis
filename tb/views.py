...
from models import TBCommentAnalysis
...
class IndexView(View):
    def get(self, request):
        ...
        return render(request, 'tb_index.html', context = {})
...
def runspider(request):
    ...
    try:
        ...
        if 'rate.taobao.com' in url and user_id != None:
            data['status'] = 'success'
            data['guid'] = str(random.randint(1000000000000, 9999999999999)) + '_' + str(random.randint(100, 999))
            ...
            if result == None:
                ...
                if force == 'false':
                    ...
                    command = "DELETE FROM {0} WHERE produce_id={1}".format(config.analysis_item_table, user_id)
                    sql.execute(command)
                    #重新分析数据
    ...
    try:
        ...
        if is_rand == 'true':
            ...
            cmd = 'cd {dir};python manage.py rand_item_analysis -a name={name} -a guid={guid}'. \
                format(dir = settings.BASE_DIR, name = 'tb', guid = data.get('guid'))
            subprocess.Popen(cmd, shell = True)
        else:
            data['info'] = '传入参数有误'
    except Exception, e:
...
def full_comment(request):
    ...
    try:
        if request.method == 'POST':
            ...
            # 检查 url 和 email 符合规范
            if url == None or 'item.taobao.com' not in url:
                data['info'] = 'URL 格式不正确，请重新输入'
            elif email == None or email == '' or '@' not in email:
                data['info'] = '邮箱格式不正确，请重新输入'
            else:
                pattern = re.compile('\d+', re.S)
                user_id = re.search(pattern, url).group()

                if 'item.taobao.com' in url and user_id != None:
                    user = {
                        'url': url,
                        'user_id': user_id,
                        'email': email,
                    ...
                    user = AnalysisUser(id = None, url = url, email = email, user_id = user_id,
                                        guid = data.get('guid'),
...
class AnalysisResultView(View):
    def get(self, request, param):
        ...
        try:
            article = TBCommentAnalysis.objects.filter(Q(guid__iexact = param) | Q(user_id__iexact = param)).first()
