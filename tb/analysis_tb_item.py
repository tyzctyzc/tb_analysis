...
from tb_analysis import settings
...
class Analysis(object):
    ...
    def init(self):
        ...
        try:
            command = "SELECT product_url, product_sku, product_title, product_id, product_price, user_nickname, " \
                      "user_viplevel,user_rank, user_anony FROM {0}". \
                format('item_%s' % self.user_id)
    ...
    def run(self):
        table_name = '%s_%s' % (config.tb_item_table, self.user_id)
        logging.warn(table_name)		
...
#        logging.warn("analysis_item_info")		
#        command = "SELECT name FROM {0} WHERE id={1}".format(config.tb_item_table, self.user_id)
#        (item_name,) = self.sql.query_one(command)

        # 商品名称
        self.record_result('# 淘宝店铺商品评价数据分析结果展示\n', default = True, br = False)

        # 商品链接
        self.record_result('淘宝商品名称：\n链接：<a href="%s" target="_blank">%s</a><br>' % (self.url, self.url),
                           default = True, new_line = True)

        # 评价数量
        command = "SELECT COUNT(*) FROM {}".format('item_%s' % self.user_id)
        (count,) = self.sql.query_one(command, commit = False)

        command = "SELECT COUNT(*) FROM {} WHERE user_viplevel>=5".format('item_%s' % self.user_id)
        (good_count,) = self.sql.query_one(command, commit = False)

        command = "SELECT COUNT(*) FROM {} WHERE user_viplevel>=3 and user_viplevel <=4".format('item_%s' % self.user_id)
        (general_count,) = self.sql.query_one(command, commit = False)

        command = "SELECT COUNT(*) FROM {} WHERE user_viplevel>=1 and user_viplevel<=2".format('item_%s' % self.user_id)
        (poor_count,) = self.sql.query_one(command, commit = False)

        command = "SELECT COUNT(*) FROM {} WHERE user_viplevel<1".format('item_%s' % self.user_id)
        (low_count,) = self.sql.query_one(command, commit = False)

        info = '总共获取评价数:%s、VIP5以上买家评价数:%s、VIP3/4买家评价数:%s、VIP1/2买家评价数:%s、VIP0买家评价数:%s' % (count, good_count, general_count, poor_count, low_count)
        logging.warn(info)		
        self.record_result(result = info, color = 'red', font_size = 24, strong = True)

        # 百分比
        info = 'VIP5以上买家评价比例:%.2f%%、VIP3/4买家评价比例:%.2f%%、VIP1/2买家评价比例:%.2f%%、VIP0买家评价比例:%.2f%%' % (
            good_count * 1.0 / count * 100, general_count * 1.0 / count * 100, poor_count * 1.0 / count * 100, low_count * 1.0 / count * 100)
        logging.warn(info)		
    ...
    def analysis_comment_info(self):
        command = "SELECT content FROM {}".format('item_%s' % self.user_id)
    ...
    def good_comment(self):
        command = "SELECT content FROM {0} WHERE score = 5".format('item_%s' % self.user_id)
        result = self.sql.query(command)
        text = ''
        for res in result:
            text += res[0]
    ...
    def general_comment(self):
        command = "SELECT content FROM {0} WHERE score=3 or score=4".format('item_%s' % self.user_id)
        result = self.sql.query(command)
        text = ''
        for res in result:
            text += res[0]
    ...
    def poor_comment(self):
        command = "SELECT content FROM {0} WHERE score<3".format('item_%s' % self.user_id)
        result = self.sql.query(command)
        text = ''
        for res in result:
            text = text + res[0]
    ...
    def analysis_content(self, contents, type):
        ...
        filename = '%s_%s.png' % (self.user_id, type)
    ...
    def analysis_buy_channel(self):
        ...
        filename = '%s_channel.png' % self.user_id
        plt.tight_layout()
    ...
    def analysis_size(self):
        ...
        plt.tight_layout()
        filename = '%s_size.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
        plt.clf()
        result = utils.get_image_src(filename = filename)
        self.record_result(result, type = 'image')
    ...
    def analysis_province(self):
        ...
        plt.tight_layout()
        filename = '%s_province.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
        plt.clf()
        result = utils.get_image_src(filename = filename)
        self.record_result(result, type = 'image')
    ...
    #
    #     plt.tight_layout()
    #     filename = '%s_province_no_other.png' % self.user_id
    #     plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
    #     plt.clf()
    #     result = utils.get_image_src(filename = filename)
    #     self.record_result(result)
    ...
    def analysis_sell_time(self):
        ...
        plt.tight_layout()
        plt.legend()
        filename = '%s_time.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
        plt.clf()
    ...
    def analysis_mobile(self):
        ...
        plt.tight_layout()
        filename = '%s_mobile.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
    ...
    def analysis_buy_days(self):
        ...
        plt.tight_layout()
        filename = '%s_days.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
        plt.clf()
    ...
    def analysis_user_level(self):
        ...
        plt.tight_layout()
        filename = '%s_user_level.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
        plt.clf()
    ...
    def analysis_hour(self):
        ...
        plt.tight_layout()
        plt.legend()
        filename = '%s_creation_time.png' % self.user_id
        plt.savefig('%s/%s' % (utils.get_save_image_path(), filename))
        plt.clf()
